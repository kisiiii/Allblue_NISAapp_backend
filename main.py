...
def get_profit_ranking(user_id: int, db: Session = Depends(get_db)):
    ...
    if user_id not in recent_user_ids:
        raise HTTPException(
            status_code=404, detail="User has no recent transactions")
    ...
    rank = next((i + 1 for i, (uid, _) in enumerate(profit_data)
                if uid == user_id), None)

    if rank is None:
        raise HTTPException(
            status_code=404, detail="User not found in ranking")
...
def get_popular_product_name(rank: int, user_id: int, db: Session = Depends(get_db)):
    ...
    sorted_products = sorted(
        product_quantities, key=lambda x: x.total_quantity, reverse=True)
...
def get_popular_product_latest_unit_price(rank: int, user_id: int, db: Session = Depends(get_db)):
    ...
    latest_unit_price = crud.get_latest_unit_price(
        target_product.product_id, db)
